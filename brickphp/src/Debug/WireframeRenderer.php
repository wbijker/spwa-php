 ...
 * in Brick::handleGet — kept entirely off the hot path, so production
 * renders never touch it.
 *
 * Mutates the tree in place:
 *
 *   - every TagDomNode picks up `.brick-wf` + a `<span class="brick-wf-label">`
...
class WireframeRenderer
{
    ...
    /**
     * <img src> → <div class="brick-wf brick-wf-img">. The original sizing
    ...
    {
        $node->setTag('div');
        self::addWireframeClass($node, 'brick-wf-img');
    ...
    /**
     * Append `.brick-wf` (and any extra class) to the node's class
    ...
    private static function addWireframeClass(TagDomNode $node, string $extraClass = ''): void
    {
        ...
        $merged = trim($existing . ' brick-wf' . ($extraClass !== '' ? ' ' . $extraClass : ''));
