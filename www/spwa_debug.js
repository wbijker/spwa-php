// SPWA debug runtime — dev-only tooling, served as a static external script.
// The server references it (<script src="/spwa_debug.js">) only on dev-mode
// pages (Config::$development), so the inspector below runs unconditionally;
// the wireframe-only behaviour self-gates on window.__SPWA_WIREFRAME, which
...
// innermost tagged element under the cursor and toggles .spwa-wf-hover for a
...
(function () {
  if (!window.__SPWA_WIREFRAME) return;
  var active = null;
